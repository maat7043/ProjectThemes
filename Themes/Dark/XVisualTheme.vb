...
Module xvDraw
    ...
    Public Sub InnerGlowRounded(ByVal G As Graphics, ByVal Rectangle As Rectangle, ByVal Degree As Integer, ByVal Colors As Color())
        ...
        Dim AddOne As Integer = 0
        For Each c In Colors
            G.DrawPath(New Pen(New SolidBrush(Color.FromArgb(c.R, c.B, c.G))), xvDraw.CreateRound(Rectangle.X + AddOne, Rectangle.Y + AddOne, Rectangle.Width - SubtractTwo, Rectangle.Height - SubtractTwo, Degree))
            SubtractTwo += 2
            AddOne += 1
        Next
    End Sub
    ...
    Protected Overrides Sub OnPaint(ByVal e As System.Windows.Forms.PaintEventArgs)
        ...
        Select Case _Shade
            Case InnerShade.Dark
                ...
                G.FillPath(InnerTexture, xvDraw.CreateRound(ClientRectangle, 3))

                G.DrawPath(GetPen(Color.FromArgb(40, 38, 36)), xvDraw.CreateRound(New Rectangle(3, 3, Width - 6, Height - 6), 3))
                G.DrawPath(GetPen(Color.FromArgb(45, 43, 41)), xvDraw.CreateRound(New Rectangle(3, 3, Width - 6, Height - 5), 3))
                G.DrawPath(GetPen(Color.FromArgb(50, 48, 46)), xvDraw.CreateRound(New Rectangle(2, 2, Width - 5, Height - 3), 3))
                ...
                G.DrawPath(hp, xvDraw.CreateRound(New Rectangle(4, 4, Width - 9, Height - 9), 3))
                ...
                G.DrawPath(op, xvDraw.CreateRound(New Rectangle(3, 3, Width - 7, Height - 7), 3))
            ...
            Case InnerShade.Light
                ...
                G.FillPath(MainGradient, xvDraw.CreateRound(ClientRectangle, 3))

                G.DrawPath(GetPen(Color.FromArgb(167, 168, 171)), xvDraw.CreateRound(New Rectangle(3, 3, Width - 6, Height - 6), 3))
                G.DrawPath(GetPen(Color.FromArgb(203, 205, 208)), xvDraw.CreateRound(New Rectangle(2, 2, Width - 5, Height - 4), 3))
    ...
    Protected Overrides Sub OnPaint(ByVal e As System.Windows.Forms.PaintEventArgs)
        ...
        Select Case _Shade
            Case InnerShade.Light
                ...
                xvDraw.InnerGlow(G, New Rectangle(1, 33, Width - 2, Height - 34), c)
            Case InnerShade.Dark
        ...
        With G
            ...
            Select Case _Style
                Case RoundingStyle.Normal
                    .FillPath(New SolidBrush(_BaseColor), xvDraw.CreateRound(Base, 5))
                    Dim tg As New LinearGradientBrush(Base, Color.FromArgb(186, 188, 191), Color.FromArgb(204, 205, 209), 90S)
                    .DrawPath(New Pen(tg), xvDraw.CreateRound(Base, 5))
                Case RoundingStyle.Rounded
                    .DrawPath(New Pen(New SolidBrush(Color.FromArgb(132, 130, 128))), xvDraw.CreateRound(New Rectangle(Base.X, Base.Y + 1, Base.Width, Base.Height - 1), 20))

                    .FillPath(New SolidBrush(_BaseColor), xvDraw.CreateRound(New Rectangle(Base.X, Base.Y, Base.Width, Base.Height - 1), 20))
                    ...
                    .DrawPath(New Pen(tg), xvDraw.CreateRound(New Rectangle(Base.X, Base.Y, Base.Width, Base.Height - 1), 20))
            End Select
        End With
...
Public Class xVisualTabControl : Inherits TabControl
    ...
    Protected Overrides Sub OnPaint(ByVal e As PaintEventArgs)
        ...
        For i = 0 To TabCount - 1
            ...
            Dim c As Color() = {Color.FromArgb(43, 40, 38), Color.FromArgb(50, 47, 44), Color.FromArgb(55, 52, 49)}
            xvDraw.InnerGlow(G, New Rectangle(124, 1, Width - 125, Height - 3), c)
        Next
